...
//CMD LINE
   ...
   {
      int [] playerscore = new int[playerobj.length];
      int x = 0;
      ...
      for(int i = 0; i < playerscore.length; i++){
       ...
       for(int i = 0; i < 13;  i++){
           for(int j = 0; j < playerobj.length; j++){
               playerscore[j] = playerscore[j] +  playerobj[j].getScoreboard(i);
               
           }
           for(int k = 0;  k < playerscore.length; k++){
               if (max <playerscore[k]){
                   max = playerscore[k];
                   x = k;
               }
      ...
      return x +1;  //x + 1 because it starts at 0
   }
